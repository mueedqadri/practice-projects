...
public class T1 implements Runnable{
    ...
    public void run(){
        try {
            ...
            System.out.println("T1 READS");
            Thread.sleep(3000);


            //----Sequence 2
            String updateQuery = "UPDATE customers set customer_city  = 'T1' where customer_id = '"+ id +"';";
            String [] list = updateQuery.split("\\s+");
            statement.executeUpdate(updateQuery);
            log.LogUpdateCustom(list,trxNumString);
            System.out.println("T1 Updates");
            Thread.sleep(3000);
