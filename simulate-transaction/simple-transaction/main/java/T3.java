...
public class T3 implements Runnable{
    ...
    public void run(){
        try {
            ...
            System.out.println("T3 READS");
            Thread.sleep(3000);


            //----Sequence 3
            String updateQuery = "UPDATE customers set customer_city  = 'T3' where customer_id = '"+ id +"';";
            String [] list = updateQuery.split("\\s+");
            statement.executeUpdate(updateQuery);
            log.LogUpdateCustom(list,trxNumString);
            System.out.println("T3 Updates");
            Thread.sleep(3000);
